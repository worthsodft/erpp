	...
	<view class="container-v main">
		<view class="banner">
			<view class="card-info" v-if="card">
				<view>卡号：<text>{{card.sn}}</text></view>
				<view>绑定有效期：<text>{{card.bind_expire_at}}</text></view>
				<view>使用有效期：<text>{{card.useful_days}}天（自绑定之日起）</text></view>
				<template v-if="card.use_type == 0">
					<view>剩余金额：<text>￥{{card.has}} 元</text></view>
				</template>
				<template v-else>
					<view>计次商品：<text>{{card.take_goods_sn_txt}}</text></view>
					<view>剩余次数：<text>{{card.has}} 次</text></view>
				</template>
			</view>
